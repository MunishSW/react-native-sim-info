 ...
 */
export const useSimInfo = () => {
  const [simSlots, setSimSlots] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState(null);
  const [hasMultipleSims, setHasMultipleSims] = useState(false);
  ...
  const requestPermission = useCallback(async () => {
    ...
    try {
      ...
      const phoneStateGranted = await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.READ_PHONE_STATE, {
      ...
      if (Platform.Version >= 33) {
        const phoneNumbersGranted = await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.READ_PHONE_NUMBERS, {
        ...
        return phoneStateGranted === PermissionsAndroid.RESULTS.GRANTED && phoneNumbersGranted === PermissionsAndroid.RESULTS.GRANTED;
      }
  ...
  const fetchSimInfo = useCallback(async () => {
    ...
    try {
      ...
      const [slots, hasMultiple, count] = await Promise.all([NativeSimInfo.getSimSlotInfo(), NativeSimInfo.hasMultipleSims(), NativeSimInfo.getActiveSimCount()]);
      setSimSlots(slots);
