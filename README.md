...
- **iOS module was unimplemented**: `SimInfo.mm` previously only contained
  a leftover template `multiply` method — none of the real spec methods
  (`getSimSlotInfo`, `hasMultipleSims`, `getActiveSimCount`,
  `addListener`/`removeListeners`) existed, which is both a
  protocol-conformance issue and a crash risk if any JS code called them.
  All spec methods are now implemented with explicit, documented iOS
  behavior: `getSimSlotInfo` rejects `NOT_SUPPORTED_IOS` (iOS has no public
  API for SIM slot enumeration), `hasMultipleSims` / `getActiveSimCount`
  resolve safe defaults (`false` / `0`), and the listener methods are
  documented no-ops.
- **`Platform.OS` guard** added on the JS side so `getSimSlotInfo` rejects
  with a clear error message on iOS instead of surfacing a raw
  native-module error.
